  ...
  if (active && payload && payload.length) {
    return (
      <div style={{
        ...
        <p className="text-sm">{payload[0].value} <span className="text-default-500">Payloads</span></p>
    ...
    <Card>
      <CardBody>
        <div className="grid grid-cols-2 items-center justify-between">
          ...
          <div className="col-span-1">
            {stats.payloads.filter((p: any) => p.count > 0).length > 0 ? (
              <div style={{ height: '60px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.payloads} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <p className="text-sm text-default-400">No payloads found for the last 7 days.</p>
            )}
          </div>
        </div>
      </CardBody>
