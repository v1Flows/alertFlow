...
func ProjectCreationStats(interval string, context *gin.Context, db *bun.DB) []models.Stats {
	var stats []models.Stats
	err := db.NewSelect().
		TableExpr("(SELECT DATE(created_at) as date, COUNT(*) as value FROM projects WHERE created_at >= NOW() - INTERVAL '"+interval+" days' GROUP BY DATE(created_at)) AS subquery").
		Scan(context, &stats)
	if err != nil {
		httperror.InternalServerError(context, "Error collecting project stats from db", err)
		return nil
	}
